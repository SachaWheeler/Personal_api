 ...
 */
	...
	$client->client_id = FITBIT_KEY; $application_line = __LINE__;
	$client->client_secret = FITBIT_SECRET;
	...
	if(($success = $client->Initialize()))
	{
		if(($success = $client->Process()))
		{
			if(strlen($client->access_token))
			{
				...
				$success = $client->CallAPI(
					'https://api.fitbit.com/1/user/-/activities/steps/date/today/1m.xml',
					'GET', array(), array('FailOnAccessError'=>true), $steps);
			}
		}
...
<?php
		...
		echo '<pre>', HtmlSpecialChars(print_r(new SimpleXMLElement($steps, 1))), '</pre>';
